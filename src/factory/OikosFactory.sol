...
interface ITokenFactory {
    function deployOikosToken(VaultDeployParams memory vaultDeployParams, address owner) external returns (OikosToken, ERC1967Proxy, bytes32);
}
...
contract OikosFactory {
    ...
    mapping(address => address) private poolToVaultMappings;
    ...
    returns (address, address, address) {
        ...
        if (
            ...
            // Only authority can execute existing token deploy
            (existingDeployData.pool != address(0) && msg.sender != authority)
        ) {
        ...
        return _finalizeVaultDeployment(
            ...
            DeploymentData({
                ...
                tokenRepo: address(0),
                vToken: address(0),
                existingVault: existingDeployData.vaultAddress
            })
        );
    }
    ...
    function _finalizeVaultDeployment(
        ...
        if (data.existingVault == address(0)) {
        ...
        poolToVaultMappings[address(data.pool)] = data.vaultAddress;
    ...
    function _deferDeploy(
        ...
        if (vaultDeployParams.presale == 1) {
            ...
            if (existingVault == address(0)) {
                ...
                // send tokens to vault and use admin functions to 
                // configure liquidity manually
            }
    ...
    */
    ...
    function getVaultFromPool(address pool) public view returns (address) {
        return poolToVaultMappings[pool];
    }
