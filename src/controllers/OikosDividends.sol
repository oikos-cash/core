...
/// @title OikosDividends
/// @notice Distribute arbitrary ERC20 dividends to holders of a "shares" token (OKS, sOKS, etc.)
...
contract OikosDividends {
    ...
    /// @notice The token whose holders receive dividends (e.g., OKS or sOKS).
    ...
    IOikosFactory public immutable factory;
    ...
    constructor(address factoryAddress, address resolverAddress) {
        ...
        factory  = IOikosFactory(factoryAddress);
        owner    = msg.sender;
    }
    ...
    /// @notice Sets the shares token from the resolver's "OikosToken" entry.
    function setSharesToken() external onlyOwner {
        address oikosTokenAddress = resolver.requireAndGetAddress(
            Utils.stringToBytes32("OikosToken"),
            "no OikosToken"
        );
        if (oikosTokenAddress != address(0)) {
            sharesToken = IERC20(oikosTokenAddress);
        }
    }
