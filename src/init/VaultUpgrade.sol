...
contract VaultUpgrade {
    ...
    function init(address _someContract, address _upgradeStep1) onlyOwner public {
        require(_someContract != address(0) && _upgradeStep1 != address(0), "Invalid address");
    ...
    function doUpgradeStart(address diamond, address _vaultUpgradeFinalize) public onlyFactory {
        ...
        IDiamondInterface(diamond).transferOwnership(_vaultUpgradeFinalize);
        IVaultUpgrader(_vaultUpgradeFinalize).doUpgradeFinalize(diamond);
    }     
