...
contract TestStaking is Test {
    ...
    function testRandomStakingAndUnstaking() public {
        ...
        for (uint i = 0; i < NUM_USERS; i++) {
            ...
            assertEq(sNOMA.balanceOf(users[i]), 0, "sNOMA balance should be 0 after unstake");
            assertGe(NOMA.balanceOf(users[i]), NOMABalanceBefore + stakeAmounts[i], "NOMA balance should be at least the initial stake amount after unstake");
        }
    ...
    function testRandomStaggeredStakingAndUnstaking() public {
        uint256[] memory stakeAmounts = new uint256[](NUM_USERS);
        uint256 totalStaked = 0;
        ...
        NOMA.mint(address(staking), additionalReward);
        vm.prank(vault);
