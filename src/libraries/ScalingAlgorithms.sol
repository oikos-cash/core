...
library ScalingAlgorithms {
    ...
    function polynomialUpdate(uint256 value, int256 rate, bool increase) public pure returns (uint256) {
        ...
        // Bound the adjustment to a reasonable percentage
        int256 maxAdjustment = int256(0.8e18); // 20%
        if (rateCubed > maxAdjustment) rateCubed = maxAdjustment;

        int256 scaleFactor = int256(1e18) + (increase ? rateCubed : -rateCubed);
        return uint256((int256(value) * scaleFactor) / int256(1e18));
    }
