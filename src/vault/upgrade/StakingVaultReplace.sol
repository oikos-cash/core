 ...
 */
contract StakingVaultReplace is BaseVault {
    ...
    function _calculateMint(
        ...
        uint256 excessReserves = IModelHelper(modelHelper())
            .getExcessReserveBalance(address(_v.pool), address(this), false);
        uint256 totalStaked = IERC20(_v.tokenInfo.token0)
            .balanceOf(_v.stakingContract);
        uint256 intrinsicMin;
        uint256 circulating = IModelHelper(modelHelper())
            .getCirculatingSupply(addresses.pool, address(this));

        try IModelHelper(modelHelper()).getIntrinsicMinimumValue(address(this)) returns (uint256 val) {
            intrinsicMin = val;
        } catch {
            revert("Failed to fetch intrinsic minimum value");
        }

        try IRewardsCalculator(rewardsCalculator()).calculateRewards(
            RewardParams(excessReserves, circulating, totalStaked)
        ) returns (uint256 rewards) {
            toMintEth = rewards;
        } catch {
            revert("Failed to calculate rewards");
        }

        if (intrinsicMin == 0) {
            revert("Divide by zero: intrinsic minimum is zero");
        }
